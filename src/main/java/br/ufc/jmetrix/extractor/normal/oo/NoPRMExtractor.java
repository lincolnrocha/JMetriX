...
public class NoPRMExtractor extends AbstractProcessor<CtClass<?>> {
	...
	public void process(CtClass<?> element) {
		if (Util.isValid(element)) {
			...
			double numberPrivateOfMethods = element.getMethods().stream().filter(m -> m.isPrivate()).count();
			Dataset.store(qualifiedName, new Measure(Metric.NoPRM, numberPrivateOfMethods));
		}
	}
}
